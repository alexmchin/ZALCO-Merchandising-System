...
function emptyInputsSignup($rname,$uidname,$email,$password){
 ...
 if(!mysqli_stmt_prepare($stmt, $sql)){
	header("location: ../Zalco/adduser.php?error=stmtfailed");
	exit();
 }
...
$hashedPwd = password_hash($password, PASSWORD_DEFAULT);
 ...
 mysqli_stmt_execute($stmt);
 mysqli_stmt_close($stmt);
 header("location: ../Zalco/adduser.php?error=none");
exit();
 }
...
function position($uidname){
    	...
    	header("location: ../Zalco/Manager.html");
 		exit();
 		}
 		else if (str_ends_with($uidname, "me")){
 		header("location: ../Zalco/Merchandiser.html");
 		exit();
}
}
 function loginUser($conn,$uidname,$password){
 	...
 	if ($uidExists=== false){
 		header("location: ../Zalco/login.php?error=wronglogin");
 		exit();
